...
currentFolder=pwd;
if nargin < 1
    [file, path, ~] = ...%file is now a cell array; use {} to index it
    uigetfile('*.rhs', 'Select an RHS2000 Data File', ...
                    'MultiSelect', 'on');% BMSEED 12/28/21-turned ON
else
    [file, path, ~] = ...
    uigetfile([newDir '*.rhs'], 'Select an RHS2000 Data File', ...
                    'MultiSelect', 'on');% BMSEED 12/28/21-turned ON
end
...
% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ %
% ~~~~ Start of while loop to go through all selected files. 			 ~~~~ %
% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ %
file_index = 1;			% index to iterate through the file selection
						% BMSEED edit 12/28/21: added while loop to iterate 
						%           through multiple files (via a cell array) 

while file_index <= length(file)

	tic;
	if isa(file, 'cell'),     filename = [path,file{file_index}];
  else,                     filename = [path,file];
  end
	...
	if exist('amplifier_data', 'var') == 0,     amplifier_data = [];    end
	if exist('board_adc_data', 'var') == 0,     board_adc_data = [];    end
	if exist('t', 'var') == 0,                  t = [];                 end
	if exist('stim_data', 'var') == 0,          stim_data = [];         end


	% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ %
	% Store ouput variables on a structure corresponding to their entry according to
	% the file order in the directory
						% BMSEED edit 12/28/21: added while loop to iterate 
						%           through multiple files (via a cell array) 
	
	Summary_var(file_index).amplifier_channels = amplifier_channels; 	% amplifier channels info
	Summary_var(file_index).amplifier_data = amplifier_data; 			    % amplifier data
	Summary_var(file_index).fqz_param = frequency_parameters;			    % frequency parameters
	
	Summary_var(file_index).b_adc_ch = board_adc_channels; 	% analog input chs info
	Summary_var(file_index).b_adc_dat = board_adc_data;			% analog input data
	
	Summary_var(file_index).t = t;										      % time data
	
	Summary_var(file_index).Stim_data = stim_data;					% stimulus data
	Summary_var(file_index).Stim_param = stim_parameters;		% stimulus parameters
	
  if isa(file, 'cell'),     Summary_var(file_index).file = file{file_index};  % files info
  else,                     Summary_var(file_index).file = file;
  end
...
% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ %
% ~~~~ End of while loop to go through all selected files. 	   		     ~~~~ %
% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ %
